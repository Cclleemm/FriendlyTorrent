...
class StorageWrapper:
    def __init__(self, storage, request_size, hashes, 
        ...
        self.initialize_tasks = [
            ['verification', 0, self.init_hashcheck, self.hashcheckfunc],
            ['fichier bouge', 1, self.init_movedata, self.movedatafunc],
            ['allocation espace disque', 1, self.init_alloc, self.allocfunc] ]
    ...
    def movedatafunc(self):
        ...
        if self.double_check and self.have[i]:
            if self.triple_check:
                ...
                if old is None:
                    return None
            if sha1(old[:]).digest() != self.hashes[i]:
    ...
    def _move_piece(self, index, newpos):
        ...
        if self.have[index] and (
            ...
            if self.triple_check:
                ...
                if old is None:
                    return -1
            if sha1(old[:]).digest() != self.hashes[index]:
                self.failed('download corrupted; please restart and resume')
                return -1
        old.release()
    ...
    def doublecheck_data(self, pieces_to_check):
        ...
        for index in sources:
            if self.have[index]:
                ...
                if piece is None:
                    return False
                if sha1(piece[:]).digest() != self.hashes[index]:
                    self.failed('download corrupted; please restart and resume')
                    return False
                piece.release()
        return True
